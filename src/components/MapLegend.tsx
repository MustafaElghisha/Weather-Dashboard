...
export default function MapLegend({ mapType }: MapLegendProps) {
  ...
  const maxValue = data.stops.at(-1)!.value;
  ...
  return (
    <div className="bg-background/50 border-accent/70 absolute top-4 right-4 flex w-[min(50%,400px)] flex-col gap-3 rounded-xl border p-4 shadow-lg">
      <h3 className="text-foreground text-sm font-semibold">{data.label}</h3>
      <div
        className="outline-accent/70 h-6 w-full rounded-xl outline"
        style={{ background: `linear-gradient(to right, ${gradientStops})` }}
      />
      <div className="text-foreground flex justify-between text-xs">
        <span>{data.stops.at(0)!.value}</span>
        <span>{data.stops.at(-1)!.value}</span>
      </div>
    </div>
  );
}

const MAP_TYPES_DATA = {
  precipitation: {
    label: "Classic Rain",
    unit: "mm",
    stops: [
